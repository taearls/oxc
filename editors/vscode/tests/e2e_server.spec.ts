...
teardown(async () => {
  ...
  await workspace
    ...
    .update("defaultFormatter", undefined);
...
suite("E2E Diagnostics", () => {
  ...
  testSingleFolderMode(
    "changing oxc.tsConfigPath will revalidate the diagnostics",
    async () => {
      ...
      await workspace
        ...
        .update("tsConfigPath", "fixtures/deep/tsconfig.json");
      await workspace.saveAll();
      await waitForDiagnosticChange();
      ...
      strictEqual(secondDiagnostics[0].code.target.authority, "oxc.rs");
      assert(
        secondDiagnostics[0].message.startsWith("Dependency cycle detected"),
      );
      strictEqual(secondDiagnostics[0].severity, DiagnosticSeverity.Error);
    },
  );
  ...
  test("formats code with `oxc.fmt.experimental`", async () => {
    ...
    await workspace
      .getConfiguration("editor")
      .update("defaultFormatter", "oxc.oxc-vscode");
