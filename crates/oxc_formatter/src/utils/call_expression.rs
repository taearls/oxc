...
pub fn is_test_call_expression(call: &AstNode<CallExpression<'_>>) -> bool {
    ...
    match (args.next(), args.next(), args.next()) {
        (Some(argument), None, None) if arguments.len() == 1 => {
            // After removing AstKind::Argument, the parent chain is now:
            //   it(CallExpression) -> async(CallExpression) -> ArrowExpression
            // instead of:
            //   it(CallExpression) -> Argument -> async(CallExpression) -> Argument -> ArrowExpression
            // So we check call.parent instead of call.parent.parent()
